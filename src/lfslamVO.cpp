...
#define ORB_DETECTOR
//#define POSE_ESTI
Point2d pixel2cam ( const Point2d& p, const CAMERA_INTRINSIC_PARAMETERS& K );
...
int main( int argc, char** argv )
{
    ...
    */
	...
	fileRead.open("/home/jdy/project/lfslamVO/data/dataset/disparity_85.txt",
			ios::in);
    cv::Mat disparityMap = cv::Mat::zeros(depth1.rows, depth1.cols, CV_64FC1);
    /*
    ...
    for (int i = 0; i < disparityMap.rows; ++i)
    {
    	for (int j = 0; j < disparityMap.cols; ++j)
    	{
    		...
    		disparityMap.at<double>(i,j) = tempLine;
    		}
    		else
    ...
    fileRead.close();
    ////////////////////// disparity to depthMap
    ///////////////////// image to point clouds
    cv::Mat depthMap = cv::Mat::zeros(depth1.rows, depth1.cols, CV_64FC1);
    double labeluint = 0.02;
    int labelnum = 75;

    PointCloud::Ptr pointCloud(new PointCloud);
    disparityLabel2depth(disparityMap, depthMap, lf_intrinsic, labeluint, labelnum,
    		rgb1, pointCloud);
    pcl::io::savePCDFileBinary(
      		"/home/jdy/project/pointXYZRGB/data/B03/pcloud_0085.pcd", *pointCloud);
    /////////////////////////
    PointCloud::Ptr pointCloud2(new PointCloud);
    image2ColorDepthMap(rgb1, depth1, pointCloud2);
    pcl::io::savePCDFileBinary(
          		"/home/jdy/project/pointXYZRGB/data/B03/pcloud_0085_disparity.pcd", *pointCloud2);
    ...
    PointCloud::Ptr pointCloud3(new PointCloud);
    image2ColorDepthMap(rgbtest, depthtest, pointCloud3);
    pcl::io::savePCDFileBinary(
      		"/home/jdy/project/pointXYZRGB/data/B03/pcloud_smallduck_disparity.pcd", *pointCloud3);
    /////////////////////////

    std::ofstream openfile("depthMap.txt");
    for (int i = 0; i < depthMap.rows; ++i)
       {
       	for (int j = 0; j < depthMap.cols; ++j)
       	{
    			double temp_d_double = depthMap.ptr<double>(i)[j];
    ...
    cv::minMaxLoc(depthMap, &minDepth, &maxDepth, &minLoc, &maxLoc);
    cv::Mat temp2 = cv::Mat::zeros(depthMap.rows, depthMap.cols, CV_8UC1);
    cout<<"maxDepth = "<<" "<<maxDepth<<endl;
    depthMap.convertTo(temp2, CV_8U, 255/2, 0);
/*
    ...
    std::ofstream openfile2("depthMapInt.txt");
        for (int i = 0; i < temp2.rows; ++i)
           {
           	for (int j = 0; j < temp2.cols; ++j)
           	{
        			double temp_d_double = temp2.ptr<double>(i)[j];
        			//int temp_d_int = (int)temp_d_double;
        			openfile2 << " " << temp_d_double;
        			//openfile << " " << temp_d_int;
    ...
    for ( DMatch m:matches )
    {
        ...
        if ( d1==0 || d2==0 )   // bad depth
            continue;
        Point2d p1 = pixel2cam ( keypoints_1[m.queryIdx].pt, K_camera );
        Point2d p2 = pixel2cam ( keypoints_2[m.trainIdx].pt, K_camera );
